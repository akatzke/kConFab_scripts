...
if ! [[ -d "./output_dorado" ]];then
	echo "output_dorado doesn't exist"
	exit 1
fi
if ! [[ -d $OUT_DIR ]];then
...
SAMPLES="/home/MHH_NGS_Daten/Projects/MinIon/HerediVar_RNASeq/kConFab_BRCA1/Samples_dorado.tsv"
if [[ -f $OUTPUT_QUA ]];then
	echo "Skipping; File already exists"
else
	if ! [[ -f "./output_dorado/call.fastq.gz" ]];then
        echo "call.fastq.gz can not be found. Please create before continuing"
	fi
	if ! [[ -f $SAMPLES ]];then
